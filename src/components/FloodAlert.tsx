...
const FloodAlert = ({ waterLevel, timestamp }: FloodAlertProps) => {
  ...
  return (
    <section className="mb-12">
      ...
      <Card className={`p-8 text-center transition-all duration-500 ${isDanger
          ? "bg-destructive/10 border-destructive"
          : "bg-success/10 border-success"
        }`}>
        <div className="max-w-2xl mx-auto">
          <div className="mb-6">
            <div className={`inline-flex items-center justify-center w-32 h-32 rounded-full mb-4 ${isDanger ? "bg-destructive/20" : "bg-success/20"
              }`}>
              <span className={`text-5xl font-bold ${isDanger ? "text-destructive" : "text-success"
                }`}>
